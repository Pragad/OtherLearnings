...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
			if ((!isPrevCharVowel) &&
				(Character.toLowerCase(word.charAt(i)) == 'a' || 
				 Character.toLowerCase(word.charAt(i)) == 'e' ||
	  			 ...
	  			 Character.toLowerCase(word.charAt(i)) == 'y'))
			{
				// 1) If we are at the last character AND
				// 2) If the character is 'e' AND
				// 3) If we have other syllables THEN
				// break out.
				if (i == word.length() - 1 &&
					word.charAt(i) == 'e' && 
					numSyllables != 0)
