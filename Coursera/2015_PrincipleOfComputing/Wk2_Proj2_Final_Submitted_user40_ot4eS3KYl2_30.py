...
class TwentyFortyEight:
    ...
    def __init__(self, grid_height, grid_width):
        """
        This function create an empty grid and then calls reset()
        to create a grid of a specific size
        """
    ...
    def move(self, direction):
        ...
        # We should take the size of the other side to traverse
        # the list of _intial_tiles
        if (direction == UP or direction == DOWN):
            size_of_temp_list = len(self._initial_tiles_dict[direction + 2])
        else:
            size_of_temp_list = len(self._initial_tiles_dict[direction - 2])
      
        # Copy grid to check for changes after the move()
        old_grid = []
        ...
        for count in range(len(self._initial_tiles_dict[direction])):
            ...
            # Time to do the merge on the temp_list. This should 
            # be updated to the grid.
        ...
        # to the existing grid. Should do a deep copy to copy the list
        # But cokeskulptor does not support "import copy"
        new_grid = []
        for item in self._grid:
...
# Create tests to check the correctness of your code. import test 
# suite and run.
# import user40_bJHRdpmDGW_19 as poc_2048_testsuite
# poc_2048_testsuite.run_suite(TwentyFortyEight)
 
# Commenting out the GUI that runs the program
#import poc_2048_gui
