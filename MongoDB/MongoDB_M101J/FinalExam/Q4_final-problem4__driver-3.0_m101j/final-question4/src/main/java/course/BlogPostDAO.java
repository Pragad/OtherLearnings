...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
        ...
        //
        //
        //postsCollection.updateOne(eq("permalink", permalink), Updates.inc("num_likes", 1));

        Document post = postsCollection.find(eq("permalink", permalink)).first();
        // fix up the likes for the comment
        if (post != null) {
            List<Document> comments = (List<Document>) post.get("comments");
            int count = 0;
            for (Document comment : comments) {
                if (ordinal == count) {
                    ...
                    //comment.put
                    //postsCollection.updateOne(eq("permalink", permalink), Updates.inc("num_likes", numLikes));
                    postsCollection.updateOne(eq("permalink", permalink), comment);
                    break;
                }
                count++;
            }
